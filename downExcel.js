...
// 生成合并单元格数据
const getMerges = (row, rowIndex, config) => {
...
const downExcel = (json, config, showError) => {
  jsonList = []
  // 判断是否为多表操作
...
// sheet页数据格式化
const formatSheetData = (json, config, showError) => {
  json = json.map((row, rowIndex) => {
  ...
  json.map((v, i) =>
    ...
    .forEach((v, i) => {
      let content = v.v
      let cell = {v: content || '', s: v.s || cellStyle}
